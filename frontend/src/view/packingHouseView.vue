...
async function refreshLots() {
  ...
  try {
    ...
    // resolve display names for "from" addresses
    ...
    <main class="main">
      ...
      <div v-if="activePage === 'pending'" class="content">
        ...
        <transition name="fade">
          <div v-if="confirmSuccess" class="alert-success">✅ LOT #{{ formatLotId(confirmSuccess) }} received — ownership transferred on blockchain</div>
        </transition>
          ...
          <div v-for="item in pending" :key="item.lotId" class="lot-card">
            ...
            <div class="onchain-preview" style="margin-top:12px">
              ...
              <div class="preview-grid">
                ...
                <div class="preview-item"><span class="preview-key">status</span><span class="preview-val">ReceivedByPackingHouse</span></div>
              </div>
            </div>
            <button class="btn-primary" style="margin-top:14px" @click="confirmReceipt(item.lotId)">Confirm Receipt (Sign Transaction)</button>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="activePage === 'inventory'" class="content">
        ...
        <div v-else class="lot-list">
          <div v-for="item in inventory" :key="item.lotId" class="lot-card">
            <div class="lot-card-top">
              <span class="lot-id">LOT #{{ formatLotId(item.lotId) }}</span>
              <span class="status-badge" style="background:#e8f5e9;color:#2d6a4f;border:1px solid #a5d6a7">At Packing
                House</span>
            </div>
            <div class="lot-variety">{{ item.variety }}</div>
            <div class="lot-info-grid">
        ...
        <div class="form-card">
          ...
          <div class="field">
            ...
            <RecipientPicker
              role="Transporter"
              v-model="transferTo"
              placeholder="พิมชื่อบริษัทขนส่ง หรือ 0x..."
            />
          ...
          <div class="onchain-preview">
            ...
            <div class="preview-grid">
              ...
              <div class="preview-item"><span class="preview-key">_to (Transporter)</span><span
                  class="preview-val mono">{{ transferTo ? shortAddress(transferTo) : '—' }}</span></div>
            </div>
          </div>
